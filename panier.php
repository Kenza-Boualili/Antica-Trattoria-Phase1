...
$plats = lirePlats();
$platsById = array_column($plats, null, 'id');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['quantite'])) {
    foreach ($_POST['quantite'] as $platId => $qte) {
        $platId = intval($platId); $qte = intval($qte);
        if ($qte <= 0) unset($_SESSION['panier'][$platId]);
        elseif (isset($_SESSION['panier'][$platId])) $_SESSION['panier'][$platId]['quantite'] = $qte;
    }
    header('Location: panier.php'); exit;
}
...
$total = array_sum(array_map(fn($i) => $i['prix'] * $i['quantite'], $_SESSION['panier']));
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <style>
        ...
        .panier-vide { text-align: center; padding: 60px 20px; color: #999; }
        .panier-vide p { margin-bottom: 20px; font-size: 16px; }
        .panier-table { width: 100%; border-collapse: collapse; font-size: 14px; }
        .panier-table th {
            text-align: left; padding: 10px; background: var(--color-beige);
            color: var(--color-bordeaux); text-transform: uppercase;
            font-size: 12px; letter-spacing: 1px;
        }
        .panier-table td { padding: 15px 10px; border-bottom: 1px solid var(--color-beige); vertical-align: middle; }
        .qte-input {
            width: 60px; padding: 5px; border: 1px solid var(--color-gold);
            text-align: center; font-family: var(--font-main); font-size: 14px;
        }
        .btn-supprimer { background: none; border: none; color: #c0392b; cursor: pointer; font-size: 18px; padding: 0 5px; }
        .btn-update {
            background: var(--color-bordeaux); color: #fff; border: none;
            padding: 10px 20px; cursor: pointer; font-family: var(--font-main);
        ...
        .resume-ligne {
            ...
            padding: 10px 0; border-bottom: 1px solid var(--color-beige); font-size: 14px;
        }
        .resume-total {
            display: flex; justify-content: space-between;
        ...
        .btn-commander {
            display: block; width: 100%; background: var(--color-bordeaux); color: #fff;
            border: none; padding: 15px; font-family: var(--font-main); font-size: 14px;
            text-transform: uppercase; letter-spacing: 2px; cursor: pointer;
            margin-top: 20px; text-align: center; text-decoration: none; transition: background 0.3s;
        }
        .btn-commander:hover { background: var(--color-gold); }
        .btn-vider { display: block; text-align: center; color: #c0392b; font-size: 13px; margin-top: 10px; text-decoration: underline; cursor: pointer; }
        .btn-continuer { display: inline-block; margin-top: 15px; color: var(--color-gold); text-decoration: underline; font-size: 14px; }
        @media (max-width: 768px) { .panier-section { flex-direction: column; padding: 30px 20px; } }
    </style>
</head>
<body>
...
<main>
    <div class="panier-hero"><h1>Mon Panier</h1></div>
                ...
                <?php else: ?>
                    <form method="POST" action="panier.php">
                        ...
                        <table class="panier-table">
                            <thead>
                                <tr>
                                    <th>Article</th><th>Prix unitaire</th>
                                    <th>Quantité</th><th>Sous-total</th><th></th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" class="qte-input"
                                                   ...
                                                   value="<?= $item['quantite'] ?>" min="0" max="20">
                                        </td>
                                        <td><strong><?= number_format($item['prix'] * $item['quantite'], 2, ',', '') ?> €</strong></td>
                                        <td>
                                            <a href="panier.php?supprimer=<?= $item['id'] ?>">
